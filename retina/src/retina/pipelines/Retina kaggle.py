...
if __name__ == '__main__':
    ...
    input_path = './01_raw/Retina Blood Vessel Segmentation kaggle/'
    ...
    logging.info('Plotting images and masks.')
    plot_images([train_raw_photos[:5], train_masks[:5]], 'Image and Mask', output_path, 'image_and_mask.png')
    logging.info('Images and masks plotted.')

    logging.info('Creating training dataset with size=5 and step=5.')
    train_features, train_labels = create_dataset(train_photos[:10], train_masks[:10], size=5, step=5)
    logging.info('Dataset created.')

    logging.info('Applying undersampling on the dataset.')
    train_features_under, train_labels_under = undersampling(train_features, train_labels)
    logging.info('Undersampling completed. Class 0: %d, Class 255: %d', train_labels_under.count(0), train_labels_under.count(255))
    ...
    y_pred_images = predict_model(knn_classifier, test_photos[:5], test_masks[:5])
